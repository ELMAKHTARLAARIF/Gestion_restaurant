...
<head>
      ...
      theme: {
        extend: {
          ...
          animation: {
            ...
            'scale-in': 'scale-in .32s ease both',
            'toast': 'toast .38s ease both',
  ...
  <style>
    ...
    /* ── Cart drawer (hidden by default, slide in) ──────── */
    .cart-drawer {
      transform: translateX(100%);
    ...
    .stg .reveal:nth-child(4) {
      transition-delay: 0.22s;
    }

    .stg .reveal:nth-child(5) {
      transition-delay: 0.28s;
    }

    .stg .reveal:nth-child(6) {
      transition-delay: 0.34s;
    }

    .stg .reveal:nth-child(7) {
      transition-delay: 0.40s;
    }

    .stg .reveal:nth-child(8) {
      transition-delay: 0.46s;
    }

    .stg .reveal:nth-child(n+9) {
...
<body class="bg-ink text-ivory font-sans overflow-x-hidden" style="font-size:14px">
  ...
  <section class="relative h-[52vh] min-h-[380px] flex items-end overflow-hidden">
    <div class="absolute inset-0">
      <img src="https://images.unsplash.com/photo-1414235077428-338989a2e8c0?w=1800&q=85" class="w-full h-full object-cover brightness-[.22]" style="animation:fu2 1s ease both" alt="" />
    </div>
  ...
  <div id="filterBar" class="sticky top-[64px] z-40 bg-ink/97 backdrop-blur-md border-b border-gold/[.1]">
    <div class="max-w-7xl mx-auto px-6 lg:px-14 flex items-center gap-0 overflow-x-auto py-0 no-scrollbar">
      ...
      <div class="relative flex-shrink-0 py-2.5">
        ...
        <input id="searchBox" type="text" placeholder="Rechercher un plat…" oninput="applyFilters()" class="inp bg-s2 border border-gold/[.1] pl-8 pr-4 py-2 w-52 text-[12px] text-ivory placeholder-ivory/20 transition-all font-sans" />
      </div>
    ...
    <div class="bg-s2 px-6 lg:px-14 py-1.5 flex items-center justify-between max-w-7xl mx-auto w-full">
      <span id="itemCount" class="text-[10px] text-ivory/25 tracking-wide">12 plats affichés</span>
        ...
        <div id="productsGrid" class="grid sm:grid-cols-2 xl:grid-cols-3 gap-px bg-gold/[.06] stg">
          ...
          <article class="prod shine card-line relative bg-s1 flex flex-col overflow-hidden group reveal cursor-pointer"
            ...
            <div class="img-z h-52 relative">
              <img src="{{ asset('storage/'.$item->image) }}" class="w-full h-full object-cover brightness-75 group-hover:brightness-90 transition-all duration-500" alt="{{ $item->name }}" />
      ...
      <aside id="stickyCart" class="hidden xl:flex flex-col w-[320px] shrink-0 sticky top-[128px] bg-s1 border border-gold/[.12] shadow-2xl">
        ...
        <div id="sCartSummary" class="hidden border-t border-gold/[.1] px-5 py-4 bg-s2">
          ...
          <button onclick="clearCart()"
            class="w-full py-2 text-[10px] tracking-[.12em] uppercase text-ivory/20 hover:text-rose transition-colors bg-transparent border-0 cursor-pointer">
            Vider le panier
          </button>
        </div>
      </aside>
  ...
  <div id="cartDrawer" class="cart-drawer fixed top-0 right-0 h-full w-full sm:w-[380px] bg-s1 border-l border-gold/[.1] z-[90] flex flex-col shadow-2xl xl:hidden">
    ...
    <div class="px-6 py-3 border-b border-gold/[.07] bg-s2">
      ...
      <select class="inp w-full bg-s1 border border-gold/[.1] px-3 py-2.5 text-[12px] text-ivory font-sans cursor-pointer">
  ...
  <div id="payModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center px-4 py-8">
    ...
    <div id="payBox" class="relative bg-s1 border border-gold/[.18] w-full max-w-lg shadow-2xl animate-scale-in overflow-y-auto" style="max-height:92vh">
        ...
        <div class="flex flex-col gap-3.5">
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <input id="f_prenom" type="text" placeholder="Ahmed" class="inp w-full bg-s2 border border-gold/[.1] px-4 py-3 text-[13px] text-ivory placeholder-ivory/20 font-sans transition-all" />
            </div>
            <div>
              <label class="block text-[9px] tracking-[.2em] uppercase text-ivory/25 mb-1.5">Nom *</label>
              <input id="f_nom" type="text" placeholder="Benali" class="inp w-full bg-s2 border border-gold/[.1] px-4 py-3 text-[13px] text-ivory placeholder-ivory/20 font-sans transition-all" />
            </div>
          </div>
          <div>
            <label class="block text-[9px] tracking-[.2em] uppercase text-ivory/25 mb-1.5">Email *</label>
            <input id="f_email" type="email" placeholder="vous@exemple.com" class="inp w-full bg-s2 border border-gold/[.1] px-4 py-3 text-[13px] text-ivory placeholder-ivory/20 font-sans transition-all" />
          </div>
          <div>
            <label class="block text-[9px] tracking-[.2em] uppercase text-ivory/25 mb-1.5">Téléphone *</label>
            <input id="f_tel" type="tel" placeholder="+212 6XX XXX XXX" class="inp w-full bg-s2 border border-gold/[.1] px-4 py-3 text-[13px] text-ivory placeholder-ivory/20 font-sans transition-all" />
          </div>
          <div>
            <label class="block text-[9px] tracking-[.2em] uppercase text-ivory/25 mb-1.5">Notes spéciales</label>
            <textarea id="f_notes" rows="2" placeholder="Allergie, demande particulière…" class="inp w-full bg-s2 border border-gold/[.1] px-4 py-3 text-[13px] text-ivory placeholder-ivory/20 font-sans resize-none transition-all"></textarea>
          </div>
        </div>
        <div class="flex gap-3 mt-5">
          <button onclick="goStep(1)" class="flex-1 py-3.5 border border-gold/20 text-ivory/45 text-[11px] tracking-[.13em] uppercase hover:border-gold hover:text-ivory transition-all bg-transparent cursor-pointer">← Retour</button>
          <button onclick="validateStep2()" class="group flex-1 py-3.5 bg-gold text-ink text-[11px] tracking-[.13em] uppercase font-semibold hover:bg-gh transition-colors border-0 cursor-pointer flex items-center justify-center gap-2">
            Continuer
            <svg class="w-4 h-4 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 8l4 4m0 0l-4 4m4-4H3" />
            </svg>
      ...
      <div id="step3" class="hidden px-7 py-6">
        <div class="text-[10px] tracking-[.3em] uppercase text-gold mb-5">Méthode de paiement</div>
        ...
        <div id="cardForm" class="flex flex-col gap-3.5">
          <div>
            <label class="block text-[9px] tracking-[.2em] uppercase text-ivory/25 mb-1.5">Numéro de carte *</label>
            <div class="relative">
              <input type="text" placeholder="1234 5678 9012 3456" maxlength="19" oninput="fmtCard(this)" class="inp w-full bg-s2 border border-gold/[.1] px-4 py-3 text-[13px] text-ivory placeholder-ivory/20 font-sans transition-all pr-16" />
              <div class="absolute right-3 top-1/2 -translate-y-1/2 flex gap-1">
                <div class="w-7 h-5 bg-blue-600/80 rounded-sm flex items-center justify-center text-[8px] text-white font-bold">VISA</div>
                <div class="w-7 h-5 bg-red-600/60 rounded-sm flex items-center justify-center text-[7px] text-white font-bold">MC</div>
              </div>
            </div>
          </div>
          <div>
            <label class="block text-[9px] tracking-[.2em] uppercase text-ivory/25 mb-1.5">Nom sur la carte *</label>
            <input type="text" placeholder="AHMED BENALI" class="inp w-full bg-s2 border border-gold/[.1] px-4 py-3 text-[13px] text-ivory placeholder-ivory/20 font-sans transition-all uppercase" />
          </div>
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-[9px] tracking-[.2em] uppercase text-ivory/25 mb-1.5">Expiration *</label>
              <input type="text" placeholder="MM / AA" maxlength="7" oninput="fmtExp(this)" class="inp w-full bg-s2 border border-gold/[.1] px-4 py-3 text-[13px] text-ivory placeholder-ivory/20 font-sans transition-all" />
            </div>
            <div>
              <label class="block text-[9px] tracking-[.2em] uppercase text-ivory/25 mb-1.5">CVV *</label>
              <div class="relative">
                <input id="cvvInp" type="password" placeholder="•••" maxlength="4" class="inp w-full bg-s2 border border-gold/[.1] px-4 py-3 text-[13px] text-ivory placeholder-ivory/20 font-sans transition-all pr-10" />
                <button onclick="toggleCvv()" class="absolute right-3 top-1/2 -translate-y-1/2 text-ivory/20 hover:text-ivory/50 bg-transparent border-0 cursor-pointer text-sm">👁</button>
              </div>
            </div>
          </div>
          <div class="flex items-center gap-2 bg-sage/5 border border-sage/15 px-4 py-2.5">
            ...
            <span class="text-[10px] text-sage/70">Paiement sécurisé SSL · Données chiffrées 256-bit</span>
          </div>
        </div>

        <div id="cashMsg" class="hidden text-center py-8 border border-gold/[.1] bg-s2">
          <div class="text-3xl mb-3">💵</div>
          <div class="text-[13px] text-ivory/60 mb-1">Paiement en espèces</div>
          <div class="text-[11px] text-ivory/30 leading-relaxed">Votre commande sera confirmée.<br>Règlement à la livraison ou sur place.</div>
        </div>
        <div id="mobileMsg" class="hidden text-center py-8 border border-gold/[.1] bg-s2">
          <div class="text-3xl mb-3">📱</div>
          <div class="text-[13px] text-ivory/60 mb-1">Paiement mobile (Wave / CIH)</div>
          <div class="text-[11px] text-ivory/30 leading-relaxed">Scannez le QR code à la caisse<br>ou envoyez au +212 600 000 000.</div>
        </div>

        <div class="flex gap-3 mt-5">
          <button onclick="goStep(2)" class="flex-1 py-3.5 border border-gold/20 text-ivory/45 text-[11px] tracking-[.13em] uppercase hover:border-gold hover:text-ivory transition-all bg-transparent cursor-pointer">← Retour</button>
          <button onclick="confirmPay()" class="group flex-[2] py-3.5 bg-gold text-ink text-[11px] tracking-[.13em] uppercase font-semibold hover:bg-gh transition-colors border-0 cursor-pointer flex items-center justify-center gap-2">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
            </svg>
            Confirmer le paiement <span id="payAmt" class="ml-1 opacity-70"></span>
          </button>
        </div>
      </div>
      ...
      <div id="step4" class="hidden px-7 py-10 text-center">
        ...
        <p class="text-[13px] text-ivory/45 leading-loose mb-2">Votre commande a été enregistrée avec succès.<br>Un email de confirmation vous sera envoyé.</p>
        <div id="orderRef" class="inline-block bg-s2 border border-gold/[.15] px-6 py-3 mb-8">
          <div class="text-[9px] tracking-[.25em] uppercase text-gold mb-0.5">Référence commande</div>
          <div class="font-serif text-lg text-ivory">#LM-2025-0042</div>
        </div>
  ...
  <div id="card-element" class="bg-s2 p-3 border border-gold/[.1]"></div>
<div id="card-errors" class="text-rose text-xs mt-2"></div>

  <form id="orderForm" action="{{ route('order.store') }}" method="POST" class="hidden">
    @csrf
    <input type="hidden" name="prenom" id="hd_prenom" />
    <input type="hidden" name="nom" id="hd_nom" />
    <input type="hidden" name="email" id="hd_email" />
    <input type="hidden" name="telephone" id="hd_tel" />
    <input type="hidden" name="mode" id="hd_mode" />
    <input type="hidden" name="notes" id="hd_notes" />
    <input type="hidden" name="payment_method" id="hd_pay_method" />
    <input type="hidden" name="total" id="hd_tot" />
    <input type="hidden" name="items" id="hd_items" />
  </form>
